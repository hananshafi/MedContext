...
from nnformer.network_architecture.tube_mask_generator import TubeMaskingGenerator
...
class Encoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.mask_generator = TubeMaskingGenerator((32,32,32),0.4)
        self.mask_token = nn.Parameter(torch.zeros(1, 1, 192))
    ...
    def forward(self, x, mask_flag=False):
        ...
        _, f,d,h,i = x.shape
        # print('#######print shape #### ....', x.shape)
        if mask_flag:
            x = x.reshape(-1,f,d*h*i).permute(0,2,1)
            B, L, _ = x.shape
            
            mask_token = self.mask_token.expand(B, L, -1)
            mask = torch.tensor(self.mask_generator())
            w = mask.flatten().unsqueeze(-1).type_as(mask_token)
            x = x * (1 - w) + mask_token * w
            x = x.permute(0,2,1).reshape(-1,f,d,h,i)
        down=[]
...
class nnFormer(SegmentationNetwork):
    ...
    def forward(self, x, mask_flag=False):
        ...
        seg_outputs=[]
        skips = self.model_down(x, mask_flag=mask_flag)
        neck=skips[-1]
