...
def test(fold):
    ...
    label_list = sorted(glob.glob('/l/users/hanan.ghani/unetr_plus_plus/UNETR_PP_DATASETS/DATASET/unetr_pp_raw/unetr_pp_raw_data/Task002_Synapse/val_labels/'+ '*nii.gz'))
    #infer_list=sorted(glob.glob(os.path.join('/l/users/hanan.ghani/nnFormer/nnformer/inference','inferTs',fold,'*nii.gz')))
    infer_list = sorted(glob.glob(os.path.join('/l/users/hanan.ghani/nnFormer/OUTPUT_SYNAPSE/output_synapse_student_teacher_weight05_mask04_normMSEMasked_4losses_1shot_iter2/nnFormer/3d_fullres/Task002_Synapse/nnFormerTrainerV2_nnformer_synapse_7__nnFormerPlansv2.1/fold_0/validation_raw_best_student_postprocessed','*nii.gz')))
    print("loading success...")
    print(label_list)
    print(infer_list)
    print(os.path.join(path,'inferTs',fold,'*nii.gz'))
    Dice_spleen=[]
    Dice_right_kidney=[]
    Dice_left_kidney=[]
    Dice_gallbladder=[]
