    ...
    def run_iteration(self, data_generator, idx, do_backprop=True, run_online_evaluation=False):
        ...
        if self.fp16:
            with autocast():
                ...
                if do_backprop:
                    output_student = self.student_network(data,mask_flag=True)
                else:
                ...
                if do_backprop:
                    l = l_student + l_teacher + 1.0*sum([self.consistency_loss(t,s) for t,s in zip(output_student,output_teacher)])
                else:
        ...
        if run_online_evaluation:
            self.run_online_evaluation(output_teacher, target)
