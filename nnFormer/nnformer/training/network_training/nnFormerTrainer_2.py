...
from nnformer.training.network_training.network_trainer_2 import NetworkTrainer
...
class nnFormerTrainer(NetworkTrainer):
    ...
    def plot_network_architecture(self):
        try:
            ...
            if torch.cuda.is_available():
                g = hl.build_graph(self.student_network, torch.rand((1, self.num_input_channels, *self.patch_size)).cuda(),
                                   transforms=None)
            else:
                g = hl.build_graph(self.student_network, torch.rand((1, self.num_input_channels, *self.patch_size)),
                                   transforms=None)
            ...
            self.print_to_log_file(self.student_network)
            self.print_to_log_file("\n")
        finally:
            if torch.cuda.is_available():
    ...
    #     assert isinstance(self.teacher_network, tuple(valid))

    #     current_mode = self.teacher_network.training
    #     self.teacher_network.eval()
    #     ret = self.teacher_network.predict_3D(data, do_mirroring=do_mirroring, mirror_axes=mirror_axes,
    #                                   use_sliding_window=use_sliding_window, step_size=step_size,
    #                                   patch_size=self.patch_size, regions_class_order=self.regions_class_order,
    #                                   use_gaussian=use_gaussian, pad_border_mode=pad_border_mode,
    #                                   pad_kwargs=pad_kwargs, all_in_gpu=all_in_gpu, verbose=verbose,
    #                                   mixed_precision=mixed_precision)
    #     self.teacher_network.train(current_mode)
    #     return ret
    ...
    def predict_preprocessed_data_return_seg_and_softmax(self, data: np.ndarray, do_mirroring: bool = True,
        ...
        valid = list((SegmentationNetwork, nn.DataParallel))
        assert isinstance(self.student_network, tuple(valid))

        current_mode = self.student_network.training
        self.student_network.eval()
        ret = self.student_network.predict_3D(data, do_mirroring=do_mirroring, mirror_axes=mirror_axes,
                                      use_sliding_window=use_sliding_window, step_size=step_size,
                                      patch_size=self.patch_size, regions_class_order=self.regions_class_order,
                                      use_gaussian=use_gaussian, pad_border_mode=pad_border_mode,
                                      pad_kwargs=pad_kwargs, all_in_gpu=all_in_gpu, verbose=verbose,
                                      mixed_precision=mixed_precision)
        self.student_network.train(current_mode)
        return ret
