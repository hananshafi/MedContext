...
class NetworkTrainer(object):
    def __init__(self, deterministic=True, fp16=False):
        ...
        self.student_network: Tuple[SegmentationNetwork, nn.DataParallel] = None
        ...
        self.momentum_schedule = cosine_scheduler(0.996, 1,
                                               self.max_num_epochs,  self.num_batches_per_epoch*2,warmup_epochs=10)
    ...
    def save_checkpoint(self, fname, save_optimizer=True):
        start_time = time()
        state_dict_student = self.student_network.state_dict()
        state_dict_teacher = self.teacher_network.state_dict()
        for key in state_dict_student.keys():
            state_dict_student[key] = state_dict_student[key].cpu()
        for key in state_dict_teacher.keys():
            state_dict_teacher[key] = state_dict_teacher[key].cpu()
        ...
        save_this = {
            'epoch': self.epoch + 1,
            'student_state_dict': state_dict_student,
            'teacher_state_dict': state_dict_teacher,
    ...
    def load_checkpoint_ram(self, checkpoint, train=True):
        ...
        new_state_dict_teacher = OrderedDict()
        curr_state_dict_student_keys = list(self.student_network.state_dict().keys())
        curr_state_dict_teacher_keys = list(self.teacher_network.state_dict().keys())
        # if state dict comes form nn.DataParallel but we use non-parallel model here then the state dict keys do not
        # match. Use heuristic to make it match
        for k, value in checkpoint['student_state_dict'].items():
            key = k
            if key not in curr_state_dict_student_keys and key.startswith('module.'):
                key = key[7:]
            new_state_dict_student[key] = value
        for k, value in checkpoint['teacher_state_dict'].items():
            key = k
            if key not in curr_state_dict_teacher_keys and key.startswith('module.'):
                key = key[7:]
            new_state_dict_teacher[key] = value
        ...
        self.student_network.load_state_dict(new_state_dict_student)
        self.teacher_network.load_state_dict(new_state_dict_teacher)
        ...
        while self.epoch < self.max_num_epochs:
            ...
            if self.use_progress_bar:
                with trange(self.num_batches_per_epoch) as tbar:
                    for b in tbar:
                        ...
                        l = self.run_iteration(self.tr_gen,idx, True)
            ...
            with torch.no_grad():
                ...
                for b in range(self.num_val_batches_per_epoch):
                    l = self.run_iteration(self.val_gen,b, False, True)
                    val_losses.append(l)
                ...
                if self.also_val_in_tr_mode:
                    ...
                    val_losses = []
                    for b in range(self.num_val_batches_per_epoch):
                        l = self.run_iteration(self.val_gen,b, False)
                        val_losses.append(l)
        ...
        if self.fp16:
            with autocast():
                ...
                if do_backprop:
                    ...
                    l = l_student + l_teacher + l_student_masked + 0.5*nn.MSELoss()(output_teacher[0],output_student_masked[0])/ torch.linalg.norm(output_teacher[0])    #0.5*entropy_loss(output_teacher[0],output_student[0], num_classes=self.num_classes)   #l_student + l_teacher +  #1.0*sum([dist_loss(t,s) for t,s in zip(output_teacher,output_student)])/3.0   #dist_loss(output_teacher[0],output_student[0])  #    #0.5*sum([self.consistency_loss(t,s) for t,s in zip(output_student,output_teacher)])
                    
                else:
                    l = l_student + l_teacher
            ...
            if do_backprop:
                l_student_masked = self.loss(output_student_masked, target)
                l = l_student + l_student_masked  + 0.5*nn.MSELoss()(output_teacher[0],output_student_masked[0])/ torch.linalg.norm(output_teacher[0])     #0.5*entropy_loss(output_teacher[0],output_student[0], num_classes=self.num_classes)    #  l_student + l_teacher #0.5*dist_loss(output_teacher[0],output_student[0])    #1.0*sum([dist_loss(t,s) for t,s in zip(output_teacher,output_student)])/3.0   #dist_loss(output_teacher[0],output_student[0])  #    #0.5*sum([self.consistency_loss(t,s) for t,s in zip(output_student,output_teacher)])
            else:
                l = l_student
