...
from nnformer.network_architecture.nnFormer_tumor_2 import nnFormer
...
class nnFormerTrainerV2_nnformer_tumor_2(nnFormerTrainer):
    """
    ...
    def run_iteration(self, data_generator,idx, do_backprop=True, run_online_evaluation=False):
        ...
        if self.fp16:
            ...
            if do_backprop:
                l_student_masked = self.loss(output_student_masked, target)
                l = l_student + l_teacher + l_student_masked  + 1.0*nn.MSELoss()(output_teacher[0],output_student_masked[0])/ torch.linalg.norm(output_teacher[0])     #0.5*entropy_loss(output_teacher[0],output_student[0], num_classes=self.num_classes)    #  l_student + l_teacher #0.5*dist_loss(output_teacher[0],output_student[0])    #1.0*sum([dist_loss(t,s) for t,s in zip(output_teacher,output_student)])/3.0   #dist_loss(output_teacher[0],output_student[0])  #    #0.5*sum([self.consistency_loss(t,s) for t,s in zip(output_student,output_teacher)])
            else:
